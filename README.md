...
Привет! Был в отпуске, возвращаюсь к проектной работе :)
<br />
По первому заданию:
<br />
1. Добавил `react-router-dom` в `auth`. У меня не было этой ошибки, т.к. в папке `frontend` у меня были тоже установлены зависимости (через `npm install`) и, видимо, в `auth` этот пакет подтягивался как `peerDependency`.
<br />
2. По поводу ошибки `Error: Cannot find module '@rspack/lite-tapable'`, проверь, пож-та, установился ли в `gallery` этот пакет `gallery/node_modules/@rspack/lite-tapable`. Если по каким-то причинам его там нет после установки, его можно просто скопировать из `node_modules` других микрофронтов (`auth` или `profile`). И если ошибка останется при присутствующем пакете, подскажи, какая у тебя версия ноды `node --version`.
</span>
